...
use sea_query::{Alias as SeaAlias, Cond as SeaCond, Condition, Expr as SeaExpr, IntoCondition, Value};

use crate::{ActflowError, store::query::*};

use super::synclient::SynClient;

pub type DbConnection = Arc<SynClient>;

fn map_db_err(err: impl std::error::Error) -> ActflowError {
    ActflowError::Store(err.to_string())
}

fn into_query(q: &Query) -> SeaCond {
    ...
    match expr.op {
        ExprOp::EQ => {
            if expr.value.is_null() {
                ...
                SeaExpr::col(SeaAlias::new(key)).eq(value.unwrap()).into_condition()
            }
        }
        ExprOp::NE => {
            if expr.value.is_null() {
                SeaExpr::col(SeaAlias::new(key)).is_not_null().into_condition()
            } else {
                SeaExpr::col(SeaAlias::new(key)).ne(value.unwrap()).into_condition()
            }
        }
        ExprOp::LT => SeaExpr::col(SeaAlias::new(key)).lt(value.unwrap()).into_condition(),
        ExprOp::LE => SeaExpr::col(SeaAlias::new(key)).lte(value.unwrap()).into_condition(),
        ExprOp::GT => SeaExpr::col(SeaAlias::new(key)).gt(value.unwrap()).into_condition(),
        ExprOp::GE => SeaExpr::col(SeaAlias::new(key)).gte(value.unwrap()).into_condition(),
    }
}
