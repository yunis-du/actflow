...
use crate::store::{Cond, DbCollection, Expr, ExprOp, PageData, map_db_err};
use crate::{ActflowError, Result, ShareLock, store::query::*};
    ...
    T: DbDocument + Send + Sync + Clone + Debug,
{
    ...
    fn find(
        ...
        self.db
            ...
            .ok_or(ActflowError::Store(format!("cannot find {} by '{}'", self.name, id)))
    }

    fn query(
        &self,
        q: &Query,
    ) -> crate::Result<PageData<Self::Item>> {
        ...
        if !q.is_cond() {
            ...
            for cond in q.queries_mut() {
                for expr in cond.conds().clone().iter() {
                    ...
                    for (k, v) in db.iter() {
                        let prop_value = v.get(expr.key()).ok_or(ActflowError::Store(format!(
                            "cannot find key `{}` in {}",
                            expr.key(),
                            self.name
                        )))?;
        ...
        if !q.order_by().is_empty() {
            rows.sort_by(|a, b| {
                ...
                for (order, rev) in q.order_by() {
                    if *rev {
                        ret = ret.then(b.get(order).unwrap().to_string().cmp(&a.get(order).unwrap().to_string()));
                    } else {
                        ret = ret.then(a.get(order).unwrap().to_string().cmp(&b.get(order).unwrap().to_string()));
                    }
                }
        ...
        let data = PageData {
            ...
            page_num,
            page_count,
            rows: rows.iter().skip(q.offset()).take(q.limit()).map(|row| map_to_model::<Self::Item>(row).unwrap()).collect::<Vec<_>>(),
        };
        Ok(data)
    }
    ...
    fn create(
        ...
        self.db.write().unwrap().insert(data.id().to_string(), data.doc()?);
        Ok(true)
    }
    ...
    fn update(
        ...
        self.db.write().unwrap().entry(data.id().to_string()).and_modify(|iter| *iter = data.doc().unwrap());
        Ok(true)
    }
