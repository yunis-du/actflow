    ...
    pub fn calc(&self) -> HashSet<Box<[u8]>> {
        ...
        for cond in self.conds.iter() {
            if result.is_empty() {
                ...
                result = result.intersection(&cond.result).cloned().collect::<HashSet<_>>()
            }
        }
        result
    }
